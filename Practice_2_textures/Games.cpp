...
void CGame::InitDirectX(HWND hWnd)
{
	...
	swapChainDesc.BufferDesc.Width = BackBufferWidth;		//lưu ý: phải tạo được fame buffer có kích thước bằng với kích thước cửa sổ
	...
	//tạo ra 1 cửa sổ 3D để giả lập 2D
	...
	//vì dùng 3D để giả lập 2D, cho nên cần ánh xạ lên mặt phẳng 2 chiều
	...
	// nói đơn giản, chuyển chế độ vẽ 3D về 2D
...
void CGame::Draw(float x, float y, LPTEXTURE tex, RECT* rect, int sprite_width, int sprite_height)
{
	...
	if (rect == NULL)
	{
		...
		if (sprite_width == 0) spriteWidth = tex->getWidth();
		if (sprite_height == 0) spriteHeight = tex->getHeight();
	}
	else
	{
	...
	DebugOut((wchar_t*)L"[INFO] Texture loaded Ok.\n");
}
